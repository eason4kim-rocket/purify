...
func TestAnswerAuthUsesErrorOnlyEnvelopeForMissingAndInvalidKeys(t *testing.T) {
	gin.SetMode(gin.TestMode)
	calls := 0
	router := gin.New()
	router.POST("/answer", AnswerAuth([]string{"required-secret"}), func(c *gin.Context) {
		calls++
		c.Status(http.StatusNoContent)
	})
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest(http.MethodPost, "/answer", nil)
			if test.header != "" {
				request.Header.Set("X-API-Key", test.header)
			}
			router.ServeHTTP(recorder, request)
			...
			if len(document) != 1 || document["error"] == nil {
				t.Fatalf("Answer auth envelope = %s", recorder.Body)
			}
		})
	}
	if calls != 0 {
...
func TestAnswerAuthAcceptsBothHeaderFormsAndSetsIdentity(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.POST("/answer", AnswerAuth([]string{"required-secret"}), func(c *gin.Context) {
		identity, exists := c.Get("api_key")
...
func TestAnswerAuthEmptyEffectiveKeySetIsInertForFailClosedRoute(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, keys := range [][]string{nil, {}, {"", " \t"}} {
		calls := 0
		router := gin.New()
		router.POST("/answer", AnswerAuth(keys), func(c *gin.Context) {
			calls++
			c.Status(http.StatusServiceUnavailable)
		})
		recorder := httptest.NewRecorder()
		router.ServeHTTP(recorder, httptest.NewRequest(http.MethodPost, "/answer", nil))
		if recorder.Code != http.StatusServiceUnavailable || calls != 1 {
			t.Fatalf("keys=%#v status/calls = %d/%d", keys, recorder.Code, calls)
		}
	}
