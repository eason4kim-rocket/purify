...
> **状态快照**：原交接 waypoint `c7189c2`（分支 `codex/search-api-v1`）；P1 N_eff v1 五卡已于 2026-08-11 完成至 `94f9f88`，整块审查补强至 `d213376`；P2 EAV 两张校准卡已完成至 `c60ec5f`，整块审查补强至 `22c379e`，Compose 配置闭合至 `1a2c601`；P3 已完成 R-1 设计定案至 `8c83af3`、R-2 pure rerank core `c8e72f8`，以及 R-3 strict runtime/config `9f837ee` + `c385830`（admission 边界澄清 `9697a3f`、网络审查补强 `e649afc`）。Phase 0–8 与 P1/P2 代码均收口；当前晋级的 curated regression replay 成绩为 P=1.000 / R=0.996 / FP=0 / U=0.003 / silent=0、hard R=0.975（完整录制口径与残余见 §15）。R-3 收口时全仓 `go test ./...`、`go test -race ./...`、`go vet ./...`、`go build ./...` 与 `git diff --check` 全绿。
...
**P3 · 步 4 重排器 + 信任排序** 🚧：PLAN.md §4.3。R-1 实况盘点与设计定案已写入 §17；R-2 pure rerank core `c8e72f8` 与 R-3 strict vLLM runtime/config `9f837ee` + `c385830` 均已落地，卡后网络审查补强为 `e649afc`，仍未接 Search/HTTP/MCP 公开 wire。R-3 的 production factory/registry 故意不存在，enabled 配置在解析 endpoint 或网络动作前固定 fail closed，须等 R-6a authenticated deployment handoff 与 R-6 manifest 同过。⟳ 交接时说“`Entity`/`fold_reason` 已备好”只表示 transport 字段存在，**不等于 Search 已有逐结果信任分**。下一步从 R-4 内部 relevance orchestration 开始，再做 component analysis + shared artifact seam，最后才开放显式 `trust` 模式。
...
> **收口状态（2026-08-12）**：N-1…N-5 五卡全部完成；生产增量按内容核、逐字血缘、可审计折叠原因三个单关注提交落地，构造集以 33 source / 12 case 对六类信号做 exact gate。分卡审计全部 PASS；随后对 `c7189c2..c12fbdc` 做整块审查，发现 MCP multi strict decoder 可被重复 `fold_reason` 的 last-wins 语义绕过，已由红测试复现并在 `d213376` 递归拒绝 exact/Unicode-escaped duplicate key，发现者复审 PASS。P2 也已完成；P3 的 R-1、R-2 与 R-3 已收口，下一步为 §17 的 R-4 内部 relevance orchestration。
