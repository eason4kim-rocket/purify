...
> **状态快照**：原交接 waypoint `c7189c2`，当前开发 waypoint `ea6d5e1`（分支 `codex/search-api-v1`）；P1 N_eff v1 五卡已于 2026-08-11 完成至 `94f9f88`，整块审查补强至 `d213376`；P2 EAV 两张校准卡已完成至 `c60ec5f`，整块审查补强至 `22c379e`，Compose 配置闭合至 `1a2c601`；P3 已完成 R-1 设计定案至 `8c83af3`、R-2 pure rerank core `c8e72f8`、R-3 strict runtime/config `9f837ee` + `c385830`（admission 边界澄清 `9697a3f`、网络审查补强 `e649afc`）、R-4 内部 relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d`；R-6 已落 provisional recording/replay/scorecard scaffold至 `5597b52`，R-6a 已落 local Docker Engine / recording-only session 与 persistent cleanup journal / crash-safe quarantine 至 `ea6d5e1`，但**尚未完成真实 judged corpus、authenticated linux/amd64 NVIDIA recordings、live mount/UDS evidence、scorecard，或 ambiguous `creating` record 的 authenticated completion barrier，因此 R-6/R-6a 均未通过**。Phase 0–8 与 P1/P2 代码均收口；当前晋级的 curated regression replay 成绩为 P=1.000 / R=0.996 / FP=0 / U=0.003 / silent=0、hard R=0.975（完整录制口径与残余见 §15）。R-6a recovery 快照全仓 `go test ./...`、package race、`go vet ./...`、`go build ./...`、linux/amd64 package build 与 `git diff --check` 全绿。
...
> **EN —** Handoff from waypoint `c7189c2`: P1 N_eff v1 and P2 EAV calibration are complete. N_eff now adds bounded content-core fingerprints, quote lineage, and additive fold-reason export on top of the existing same-root/whole-page-simhash folds. EAV now enforces verbatim extraction quotes through the golden replay gate and can opt only its operator-managed client into private networking while request BYOK remains public-only. P3's R-1 design, R-2 pure core, R-3 isolated strict runtime/config, R-4 internal relevance orchestration, and R-5 explicit relevance contract are complete. R-6 has a pinned manifest, strict offline replay/metrics, a provisional-only scorecard, and a label-blind recorder; R-6a now has a fail-closed local Docker Engine, a recording-only session, and a persistent three-state cleanup journal. The journal safely converges prepared and owned-ID records but quarantines every ambiguous creating record without Docker or host mutation. Neither gate is complete without independently judged data, authenticated linux/amd64 NVIDIA recordings and live runtime evidence, price evidence, a live 10+100 run, and a stronger authenticated completion barrier for creating records. Production activation remains closed until both gates pass.

---

## 17. P3 · 0.6B relevance reranker + trust-aware ranking（R-1 设计定案）

> **状态（2026-08-12）**：R-1…R-5 已收口；R-6 只交付 provisional manifest/replay/scorecard/recorder scaffold，真实 judged corpus、authenticated linux/amd64 GPU recordings 与 live scorecard 尚缺，因此仍未通过。v1 默认始终是 `provider`，relevance/trust 只能显式 opt-in；R-6 与 R-6a 双门未过前不得在生产启用 relevance capability，trust oracle 门未过前不得开放 `trust`。
...
R-6  ⟳ 3c98118 + 3a30a90 + d0889d9 + 8031269 / review fix 5597b52：provisional manifest/replay/scorecard/recorder scaffold；真实 judged/GPU artifacts 尚缺，未通过
