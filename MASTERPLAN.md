...
> **状态快照**：原交接 waypoint `c7189c2`，当前开发 waypoint `5597b52`（分支 `codex/search-api-v1`）；P1 N_eff v1 五卡已于 2026-08-11 完成至 `94f9f88`，整块审查补强至 `d213376`；P2 EAV 两张校准卡已完成至 `c60ec5f`，整块审查补强至 `22c379e`，Compose 配置闭合至 `1a2c601`；P3 已完成 R-1 设计定案至 `8c83af3`、R-2 pure rerank core `c8e72f8`、R-3 strict runtime/config `9f837ee` + `c385830`（admission 边界澄清 `9697a3f`、网络审查补强 `e649afc`）、R-4 内部 relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d`；R-6 已落 provisional recording/replay/scorecard scaffold `3c98118` + `3a30a90` + `d0889d9` + `8031269`，canonical manifest review-fix 为 `5597b52`，但**尚未完成真实 judged corpus、authenticated linux/amd64 GPU recordings 或 live scorecard，因此 R-6 仍未通过**。Phase 0–8 与 P1/P2 代码均收口；当前晋级的 curated regression replay 成绩为 P=1.000 / R=0.996 / FP=0 / U=0.003 / silent=0、hard R=0.975（完整录制口径与残余见 §15）。R-6 scaffold 冻结时全仓 `go test ./...`、`go test -race ./...`、`go vet ./...`、`go build ./...` 与 `git diff --check` 全绿。
...
**P3 · 步 4 重排器 + 信任排序** 🚧：PLAN.md §4.3。R-1 实况盘点与设计定案已写入 §17；R-2 pure rerank core `c8e72f8`、R-3 strict vLLM runtime/config `9f837ee` + `c385830`、R-4 internal relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d` 均已落地，R-3 卡后网络审查补强为 `e649afc`。R-6 已有 pinned tuple/asset manifest、strict three-file replay、NDCG/bootstrap、始终 provisional 的 scorecard evaluator和 label-blind recorder CLI，但仓内没有拿 synthetic gold/伪 attestation 冒充真实成绩。省略/显式 `provider` 永久保持旧排序与 wire；显式 `relevance` 已具 20-candidate fan-out、结果级 score、degraded fallback 及 HTTP/MCP strict contract，但生产 scorer factory/registry 仍故意不存在，enabled 配置在解析 endpoint 或网络动作前固定 fail closed，须等 R-6a authenticated deployment handoff 与真实 R-6 recording manifest 同过。`trust` 请求形状、默认值和最坏计费已锁，但任何成功响应在 R-8 前 fail closed；`Entity`/`fold_reason` 仍只表示 transport 字段存在，**不等于 Search 已有逐结果信任分**。下一步仍是完成 R-6：独立 judged docs/labels、authenticated linux/amd64 GPU recordings、价格证据与 10+100 live scorecard；R-6a 需提供其运行时证据链，二者均过后才轮到 R-7/R-8。
...
> **EN —** Handoff from waypoint `c7189c2`: P1 N_eff v1 and P2 EAV calibration are complete. N_eff now adds bounded content-core fingerprints, quote lineage, and additive fold-reason export on top of the existing same-root/whole-page-simhash folds. EAV now enforces verbatim extraction quotes through the golden replay gate and can opt only its operator-managed client into private networking while request BYOK remains public-only. P3's R-1 design, R-2 pure core, R-3 isolated strict runtime/config, R-4 internal relevance orchestration, and R-5 explicit relevance contract are complete. R-6 now has a pinned manifest, strict offline replay/metrics, a provisional-only scorecard, and a label-blind recorder, but it is not complete without independently judged data, authenticated linux/amd64 GPU recordings, price evidence, and a live 10+100 run. Production activation still requires both that real R-6 evidence and the separate R-6a deployment admission.
...
R-6a ⟳ local Docker Engine controller/recording-only live session；真实 NVIDIA evidence 与 R-6 manifest 同过才生成 certified registry entry
