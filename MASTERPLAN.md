...
> **状态快照**：原交接 waypoint `c7189c2`，当前代码 waypoint `682317c`（分支 `codex/search-api-v1`）；P1 N_eff v1 五卡已于 2026-08-11 完成至 `94f9f88`，整块审查补强至 `d213376`；P2 EAV 两张校准卡已完成至 `c60ec5f`，整块审查补强至 `22c379e`，Compose 配置闭合至 `1a2c601`；P3 已完成 R-1 设计定案至 `8c83af3`、R-2 pure rerank core `c8e72f8`、R-3 strict runtime/config `9f837ee` + `c385830`（admission 边界澄清 `9697a3f`、网络审查补强 `e649afc`）、R-4 内部 relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d`；R-6 已在 provisional recording/replay/scorecard scaffold 上继续落 blind judgment 与 provider-price evidence 合同至 `60eff7b`，随后 `3b89cb1→682317c` 五提交复审 PASS（盲标合同、Brave 公开牌价合同、两道认证门仍无条件 unavailable）；R-6a 已落 local Docker Engine / recording-only session 与 persistent cleanup journal / crash-safe quarantine 至 `ea6d5e1`，但**尚未完成真实 licensed judged corpus、authenticated linux/amd64 NVIDIA recordings、live mount/UDS evidence、完整价格 artifact/scorecard，或 ambiguous `creating` record 的 authenticated completion barrier，因此 R-6/R-6a 均未通过**。无 GPU 且无 Brave storage/AI 授权时，本轮只收 **seed_order** 许可语料（不持久化 Search Results、不宣称 vs Brave）、公开牌价 capture、以及 creating 的人工 adopt/abandon。Phase 0–8 与 P1/P2 代码均收口；当前晋级的 curated regression replay 成绩为 P=1.000 / R=0.996 / FP=0 / U=0.003 / silent=0、hard R=0.975（完整录制口径与残余见 §15）。
...
**P3 · 步 4 重排器 + 信任排序** 🚧：PLAN.md §4.3。R-1 实况盘点与设计定案已写入 §17；R-2 pure rerank core `c8e72f8`、R-3 strict vLLM runtime/config `9f837ee` + `c385830`、R-4 internal relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d` 均已落地，R-3 卡后网络审查补强为 `e649afc`。R-6 已有 pinned tuple/asset manifest、strict three-file replay、NDCG/bootstrap、始终 provisional 的 scorecard evaluator、label-blind recorder、独立盲标注 packet/compile/validate CLI 与 strict Brave provider-call price evidence 合同；R-6a 也已有固定本机 Docker socket、exact image/create/inspect/event、artifact/host trust、每次录制前后 live bracket 的 recording-only session，以及持久三态 cleanup journal。journal 对 `prepared`/`owned(ID)` 可安全收敛，对任何 ambiguous `creating` 只隔离保留并阻断新 session，绝不把 name 可见、404 或 operator assertion 当清理授权。两卡都没有拿 synthetic gold、fake GPU、自填价格摘要或 operator assertion 冒充真实成绩。省略/显式 `provider` 永久保持旧排序与 wire；显式 `relevance` 已具 20-candidate fan-out、结果级 score、degraded fallback 及 HTTP/MCP strict contract，但生产 scorer factory/registry 仍故意不存在，enabled 配置在解析 endpoint 或网络动作前固定 fail closed，须等 R-6a authenticated deployment handoff 与真实 R-6 recording manifest 同过。`trust` 请求形状、默认值和最坏计费已锁，但任何成功响应在 R-8 前 fail closed；`Entity`/`fold_reason` 仍只表示 transport 字段存在，**不等于 Search 已有逐结果信任分**。下一步在无 GPU、无 Brave storage/AI 授权的前提下，只收 **seed_order** 许可语料（自有 L1/公开页快照，禁止持久化 Search Results，禁止宣称 ΔNDCG vs Brave）+ 独立盲标 + Brave 公开牌价 capture + `creating` 人工 adopt/abandon。普通 Brave Search 计划的 [Search API Terms of Use](https://api-dashboard.search.brave.com/documentation/resources/terms-of-service) §3(b)(i)/(xiii) 禁止非瞬时存储 Search Results 及将其用于 AI 评测/benchmark，官方 [Search API FAQ](https://brave.com/search/api/) 也要求存储结果使用显式授予 storage rights 的计划，因此没有同时覆盖存储与 AI 评测的 Order Form/书面授权前不得用 Brave 采集本 corpus。数据授权与 NVIDIA 主机齐备后，再闭合 R-6a live mount/UDS/runtime evidence、recordings 与 10+100 scorecard；有 GPU 之前不称 terminal R-6a。之后才轮到 R-7/R-8。
...
> **收口状态（2026-08-12）**：N-1…N-5 五卡全部完成；生产增量按内容核、逐字血缘、可审计折叠原因三个单关注提交落地，构造集以 33 source / 12 case 对六类信号做 exact gate。分卡审计全部 PASS；随后对 `c7189c2..c12fbdc` 做整块审查，发现 MCP multi strict decoder 可被重复 `fold_reason` 的 last-wins 语义绕过，已由红测试复现并在 `d213376` 递归拒绝 exact/Unicode-escaped duplicate key，发现者复审 PASS。P2 也已完成；P3 的 R-1…R-5 已收口，R-6 只完成了 provisional infrastructure，下一步仍是实际 judged relevance corpus、authenticated recordings 与 live scorecard，不把 scaffold 标成评测通过。
...
> **EN —** Handoff from waypoint `c7189c2`: P1 N_eff v1 and P2 EAV calibration are complete. N_eff now adds bounded content-core fingerprints, quote lineage, and additive fold-reason export on top of the existing same-root/whole-page-simhash folds. EAV now enforces verbatim extraction quotes through the golden replay gate and can opt only its operator-managed client into private networking while request BYOK remains public-only. P3's R-1 design, R-2 pure core, R-3 isolated strict runtime/config, R-4 internal relevance orchestration, and R-5 explicit relevance contract are complete. R-6 has a pinned manifest, strict offline replay/metrics, a provisional-only scorecard, a label-blind recorder, strict blind-judgment tooling, and a provider-price evidence contract; R-6a now has a fail-closed local Docker Engine, a recording-only session, and a persistent three-state cleanup journal. The journal safely converges prepared and owned-ID records but quarantines every ambiguous creating record without Docker or host mutation. Neither gate is complete without licensed independently judged data, authenticated linux/amd64 NVIDIA recordings and live runtime evidence, real provider and GPU-price artifacts, a live 10+100 run, and a stronger authenticated completion barrier for creating records. Ordinary Brave Search terms do not authorize storing Search Results for this AI benchmark, so a qualifying Order Form or another licensed source is a prerequisite. Production activation remains closed until both gates pass.

---

## 17. P3 · 0.6B relevance reranker + trust-aware ranking（R-1 设计定案）

> **状态（2026-08-12）**：R-1…R-5 已收口；R-6 已交付 provisional manifest/replay/scorecard/recorder、blind judgment 与 provider-price evidence 合同，但真实 licensed judged corpus、官方价格 artifacts、authenticated linux/amd64 GPU recordings 与 live scorecard 尚缺，因此仍未通过。v1 默认始终是 `provider`，relevance/trust 只能显式 opt-in；R-6 与 R-6a 双门未过前不得在生产启用 relevance capability，trust oracle 门未过前不得开放 `trust`。
...
- reference runtime 只认证 `linux/amd64`：vLLM `v0.23.0`、image child `vllm/vllm-openai@sha256:3a1e7f5904e1a1192a02aa0086ceaffc33985d7044c7bb25b3a43d61bdbe3ac0`（multi-arch index 仅作 provenance：`sha256:6d8429e38e3747723ca07ee1b17972e09bb9c51c4032b266f24fb1cc3b22ed8f`）。R-6a 将 transport 收紧为容器 `network=none` 内的 Unix socket，required argv exact 为 `vllm serve Qwen/Qwen3-Reranker-0.6B --revision e61197ed45024b0ed8a2d74b80b4d909f1255473 --tokenizer-revision e61197ed45024b0ed8a2d74b80b4d909f1255473 --served-model-name Qwen/Qwen3-Reranker-0.6B --runner pooling --max-model-len 8192 --no-enable-prefix-caching --hf-overrides '{"architectures":["Qwen3ForSequenceClassification"],"classifier_from_token":["no","yes"],"is_original_qwen3_reranker":true}' --chat-template /run/purify/qwen3_reranker.jinja --uds /run/purify/vllm.sock`，**禁止**把 key 放进 argv。pinned image 的 entrypoint 已是 `vllm serve`，Docker create 固定 `Entrypoint=["vllm","serve"]`与 `Cmd=Argv[2:]`，并从 inspect 的 actual `Path+Args` 重建、比较上述全量 argv。reference profile 显式禁用 vLLM prefix cache，避免 replay/scorecard 的重复 query 借 KV 命中伪造 cold latency。挂载 template SHA-256 必须是 `e1ee98e69aab7b2da366edf1c50efcef37e34b4a0c50fb816336213e68d9047a`；model snapshot 必须以不可变 artifact 挂到 exact 容器路径 `/root/.cache/huggingface/hub/models--Qwen--Qwen3-Reranker-0.6B/snapshots/e61197ed45024b0ed8a2d74b80b4d909f1255473`，按仓内 inventory 验完整实际 container mount view，且 `HF_HUB_OFFLINE=1`，不在启动时下载 moving main。arm64/其他平台必须独立录制、定 digest 与重过 R-6，不能借 multi-arch tag 冒充已认证。
- R-3 提交的 pure env-policy 只定义 supervisor overlay，不能冒充 Docker 的完整 child env。R-6a 把 actual env 锁为“由 child manifest 与 exact image-config ID `sha256:f37691f675bb82f734f606de8af90e777d3f80a20b120e699fd43fd10e60b8d7` 认证的 pinned image baseline + exact supervisor overlay”：operator 不得提供任何 extra env，overlay name 必须在读取/序列化 value 前通过 exact allowlist，只有 required `HF_HUB_OFFLINE=1`、`VLLM_NO_USAGE_STATS=1` 与 secret `VLLM_API_KEY`；missing/wrong/duplicate telemetry opt-out fail closed。image 内建的 build/usage `VLLM_*` 只能来自已认证 baseline，不得被 operator 新增/覆盖；实际 `ContainerInspect.Config.Env` 必须等于按 name byte-order 确定性 merge 后的全集。create 还须固定 `Hostname=purify-r6a-<controller random lowerhex>`、`Tty=false`、无 links；本卡只准入 Moby legacy NVIDIA driver 对 pinned `Count=1` / `compute` request 的 exact OCI 变换：Moby 先建立默认 `PATH,HOSTNAME` 两槽，再用 `Config.Env` 覆盖同名默认 `PATH` 并按原顺序追加其余项，最后追加恰好 `NVIDIA_VISIBLE_DEVICES=0` 与 `NVIDIA_DRIVER_CAPABILITIES=compute` 两项；PID 1 envp 因而必须逐项、逐序等于 `Config.Env` 的 `PATH`、`HOSTNAME=<fixed Hostname>`、`Config.Env` 其余项、上述两项 runtime append（后两项会与 image baseline 的同名项并存，不能按 name 折叠或忽略顺序）。CDI、其他 device selection/capability 或任何额外 runtime env 都 fail closed，须另卡实录后才能放行。supervisor 直接从 host `/proc/<State.Pid>/environ` bounded 读回该实际环境，并用读取前后两次 exact same-running-PID inspect 夹住；不用 Docker exec 冒充 PID 1 environment。key由 controller 在内存生成，须满足1..16 KiB、UTF-8、无control/首尾空白，只在内存用 constant-time 比较；redacted effective env 的唯一secret仅记 `<present>`，真实key不入argv/digest/evidence/recording/log/仓库（但本机 Docker daemon/config 必然持有 child env，具有 socket/host-root 权限者明确属于可见 secret 的可信 deployment boundary）。env-policy 必须升级新 version 并纳入 recording manifest identity；UDS argv 与该 tuple 变更都会使当前 provisional manifest identity 自动失效，必须重录，不允许用旧 ID 晋级。
- **R-6a Docker Engine 定案**：actual backend 只连本机 `/var/run/docker.sock`，忽略 `DOCKER_HOST`/context/proxy 且拒绝远程 TCP；要求 Linux daemon、Engine API≥1.49、rootful/no-userns-remap daemon、controller euid=0 与 `linux/amd64`。controller 不 pull，只接受 daemon 已预载的 exact child manifest digest；API 1.49 的 containerd image store 在 platform-qualified `ImageInspect` 中把 `ID` 与 `Descriptor.digest` 都投影为该 child manifest（classic graphdriver 则缺 Descriptor，必须 fail closed），所以不能把 API 不提供的 config blob digest伪装成 observed `ImageInspect.ID`。pre-create 必须同时锁 `ID==Descriptor.digest==exact child`、platform 与从该 manifest config 解出的完整 pinned image config（config digest仍由 manifest identity密码学绑定）；随后 create 的 `Config.Image` 使用 exact named child digest，不能用 containerd store 无法解析的 config blob digest。create 后、start/session 前及后续每次 `ContainerInspect.ImageManifestDescriptor` 与 `ContainerInspect.Image` 都必须 exact child，`ContainerInspect.Config.Image` 必须 exact named child digest；任一 missing/index/config swap均 fail closed。pinned image config 的十个内建 label 与四个 controller ownership label 组成 actual `Config.Labels`；ownership 判断只投影并 exact 比较四个 `purify.*` label，同时完整 static validator exact 核对全部十四个，既不能因 image label 正常 merge 恒拒，也不能让额外 label 偷渡。controller 自己生成 run ID/spec digest、deterministic diagnostic name、create/start/inspect/monitor/kill/remove；正常生命周期与自动清理只按 Docker 返回并已持久化为 `owned` 的 container ID，且每步先复核 canonical name 与四个 ownership labels。name/spec digest 会进入持久恢复记录，但 ambiguous `creating` 状态不得因 exact-name inspect、404、冲突或特定 Moby 内部锁实现而升级为清理授权，不按 name 接管或删除现有对象。create 固定 `NetworkMode=none`、`NetworkDisabled=true`、`LogConfig=none`、runtime `runc`、`Init=false`、private cgroup namespace、零 exposed/published port、read-only rootfs、restart=no、`Privileged=false`、无 `CapAdd`、exact NVIDIA compute device request；created/running/final inspect 必须逐项认证这些值、Docker 默认 masked/readonly `/proc`/`/sys` 防护与空 network sandbox/attachment/port，daemon default ulimit 或其他资源注入一律 fail closed。应用数据的可写集合只允许 controller-created mode `0700` 的 fresh/empty host IPC run-dir、private `/dev/shm` 1 GiB，以及容器内 `/tmp` 1 GiB、`/root/.cache/vllm` 1 GiB、`/root/.triton` 512 MiB 三个 bounded tmpfs，不使用 `ipc=host`；另明确接受 stock Docker OCI 的 bounded pseudo-filesystem mounts（`/dev` 64 MiB tmpfs、devpts、mqueue 与 `/proc`），不把它们虚报成应用 artifact 写入面，live GPU gate 必须认证其 exact mountinfo。template/snapshot 都是 exact 目标的 read-only artifact，snapshot 必须是 inventory 一一对应的 materialized regular-file tree，拒绝 symlink/hardlink/device/额外文件，不直接挂标准 HF snapshot 里指向 `../../blobs` 的 symlink；host 侧 snapshot 全树及从 snapshot 到 `/` 的祖先还必须 root-owned 且 group/other 不可写，内容摘要校验前后都重验该信任边界。controller 在 start 前确认 run-dir 内无 socket/其他项，start 后 lstat 必须是该目录内的新 UDS、非 symlink，再通过它直接做 authenticated readiness 与 strict `/v1/rerank` 录制。容器全程无网络、无 host publish，因而其他 vLLM routes 不会被 TCP 公开且 outbound default-deny。Docker event stream 先用带纳秒游标的 replay 加两条 owner-bound `archive-path` acknowledgement 建立订阅 barrier，未确认前绝不 start；daemon event bus 可静默丢事件，所以 live stream 只作负向快速撤权，绝不以“没看到事件”证明健康。每次 `Record` 前后与每次 evidence 读取都重新执行完整 static/ownership/same-anchor-PID/PID1-env/UDS running bracket，任何漂移先 revoke authority、丢弃 late data，再只清理 exact owned child。本卡对 Docker daemon 与本机 root 设置明确信任边界；拥有 Docker socket 的攻击者等价 host root，不伪称 inspect 能抵御恶意 daemon/host。
- **R-3 审计纠偏 / R-6a handoff**：禁止做一个读取 operator JSON、manifest path、argv/env 或布尔网络声明便返回成功的“认证工具”。controller 只能返回由本次 actual child 活性绑定的、字段私有且不可反序列化的 recording-only `ReferenceSession`；endpoint/key 由 session 内生并直接构造 strict recorder，不向 caller 暴露，evidence 只能 redacted 输出，不得提供 Decode/Admit/从 evidence 构造 session 的逆向 API，session 也不实现 production `Scorer`。R-6a 代码在无 GPU 环境可以用 fake/CPU Docker 锁定所有 fail-closed 状态机，但在真实 NVIDIA `linux/amd64` child、实际 mount/UDS/readiness/lifecycle evidence 通过前仍标 `⟳`。cleanup 现先取得 `/var/lib/purify-r6a-recovery` 的 root-only 跨进程 lease，再以 durable `prepared → creating → owned(ID)` 三态绑定 run ID、deterministic name、snapshot path 与 secret-redacted spec digest；prepared 只清 host，owned 才能按 ID+name+四 labels 收敛 container→host→record，正常 session 全生命周期持有 lease。任意 `creating`（包括 Create 无 ID、或 ID 已返回但 owned transition 未持久化）都保留 child/host/record并阻断新 session，零 Resolve/Create/Start/Kill/Remove/host mutation；Docker API 未提供可跨实现认证旧 Create handler 已完成的 barrier，所以 name 可见或 404 都不能自动晋级。该设计关闭误删与 journal 丢失窗口，但有意选择 fail-closed quarantine，不是 terminal automatic cleanup/no-orphan；terminal R-6a 仍需更强 authenticated daemon-completion 或明确人工恢复协议。`RequireCertifiedProfile` / `RequireCertifiedDeployment` 与 production factory/registry 继续无条件 unavailable；只有 R-6a authenticated handoff 与 R-6 judged manifest/scorecard 同过才能在后续单独提交生成 registry entry。
...
- `docs.jsonl` / `labels.jsonl` / `recordings.jsonl` 分离，逐行 exact allowlist，拒 BOM/空行/非 UTF-8/duplicate/Unicode-duplicate/case-smuggle/unknown/trailing/null。三文件 case ID exact one-to-one join，拒 orphan/unused；case/query ID 非空唯一，candidate ID 唯一、provider_rank 必须刚好是 1..N，N=10..20，grade 是 0..3 整数且 IDCG@5>0。单行≤256 KiB、单文件≤16 MiB、单 string≤64 KiB、JSON depth/数组均有界。recording 保存完整 pinned manifest identity、production wire `input_digest`、另一个绑定 ordered stable-ID/provider-rank 的 `candidate_digest`、exact candidate-score key set、usage/latency；不存任意 fixture hash 来代替 production input builder，也不把 canonical identity 冒充 runtime attestation。
...
- scorecard 对同一 query/candidate 集合先做每条路径各10次 warmup（丢弃），再做100个 cold-cache / `BypassProviderCache` measured pair；sidecar 必须由 attestation 证明带 `--no-enable-prefix-caching`，否则本轮无效，不能把第2轮起的 KV hit 叫 cold。R-6a 前允许保存带 raw samples 的 **provisional** report，但任意 operator evidence ID/boolean 都不能令其 production pass。case 按 ID byte-lexical 排序，pair `i` 用 case `i mod n`；A=新鲜 provider baseline、B=同一 baseline input 的 rerank batch，偶数 pair 固定 A→B、奇数 pair B→A。不得按结果挑顺序或重试；provider cache-hit 路径另记但不进入 `rerank_ms < provider_ms` 比值，避免 provider_ms=0 空门/死门。p50/p95 用100个原始样本升序后的 nearest-rank（zero-based `[49]`/`[94]`）；记录 hardware/GPU/region/runtime image digest/model/template/revision/provider 版本、运行 UTC 时间、当日价格证据摘要与全部原始样本，不只报一个中位数。
...
R-6  ⟳ 3c98118 + 3a30a90 + d0889d9 + 8031269 / review fix 5597b52 + 3b89cb1 + 1604511 / review fix ee99dbe + 60eff7b：provisional manifest/replay/scorecard/recorder + blind judgments + provider-price evidence；真实 licensed judged/GPU/price artifacts 尚缺，未通过
R-6a ⟳ c4580e9..ea6d5e1：local Docker Engine/recording-only session + persistent cleanup journal/crash-safe quarantine；真实 NVIDIA evidence、ambiguous creating completion barrier 与 R-6 manifest 同过才生成 certified registry entry
...
> **R-6 provisional scaffold（2026-08-12，未通过 R-6）**：`3c98118 feat(search): pin rerank recording foundation` 将 vLLM v0.23.0 template、HF snapshot inventory、完整 typed tuple 与 domain-separated manifest identity 固定进仓，并从 production request builder/strict decoder抽出 recording-only observation；wire digest 与 ordered stable-ID/provider-rank candidate digest 双重绑定，防同文本候选调换 URL 后复用旧 index→score 映射，recorder 不实现 `Scorer`、不产生 production factory。`3a30a90 test(search): add pinned rerank replay gates` 新增三文件 strict JSONL、一对一 join、24/240 coverage、production total-order NDCG、完整 construction gates 与 locked SplitMix64 paired bootstrap；release significance 先过所有 construction gates再算 lower bound。`d0889d9 test(search): add provisional rerank scorecard` 锁10个 warmup pair、100个 A/B pair、raw samples、`[49]/[94]` percentile、价格证据/运行日期、候选输入双摘要与成本公式，但无论自填何种 deployment evidence 都固定 `Provisional=true`、`ProductionPassed=false`。`8031269 feat(search): add provisional rerank recorder` 只读 strict docs、不读 labels、不重试、不接受 model/profile/manifest override，输出经同目录 0600 temp + no-replace install，失败只清自己的 temp；endpoint 仍只是 operator 给出的 reference endpoint，产物在 R-6a 认证前不能称 pinned runtime recording。卡后审查发现 `-0.0` 可产生另一个自洽 manifest identity，`5597b52 fix(search): reject noncanonical rerank manifest zero` 以 IEEE 位级 exact 门闭合，复审 PASS。随后 `3b89cb1 test(search): validate blind rerank judgments` 增加 cases/docs/盲标 packet/judgments/compiled labels 的 strict join、可见字段 digest、固定 rubric、objective-hard 与至少24 cases/合计240 candidates coverage 门，`60eff7b feat(search): add blind rerank labeling command` 只将 packet/compile/validate 包成字段盲化、0600、no-replace 的离线 CLI；blind packet 保留 `case_id/query/candidate_id/title/snippet`，隐藏 URL、provider rank、language、bucket 与 hard，两者都不采集 provider 数据、不生成 synthetic gold。`1604511 test(search): bind Brave price evidence` 增加单 operand 的 strict provider-call 价格 evidence/官方页面 extractor，`ee99dbe fix(search): freeze rerank price evidence reads` 再把 artifact 文件锁定为同一读取快照；这只证明离线 evidence 合同，仓内没有真实官方 capture，也没有 GPU-hour operand。当前仍没有真实 licensed `docs.jsonl`/`labels.jsonl`、Qwen `recordings.jsonl`、provider/GPU 官方价格 artifacts 或 authenticated GPU scorecard；普通 Brave Search 计划在 §3(b)(i)/(xiii) 下不能用于本次持久化 AI benchmark，必须先取得覆盖存储与 AI 评测的明确授权或改用许可明确的数据源。因此本段仍只记 provisional waypoint，不生成 registry entry、不推进 R-6 状态；`RequireCertifiedProfile` / `RequireCertifiedDeployment` 仍无条件 unavailable。

> **R-6a provisional scaffold（2026-08-12，未通过 R-6a）**：`c4580e9` 先锁 local-only supervisor 与 R-6/R-6a 边界，`fda8a3f`、`a6721ea`、`155c24f`、`cd7903b` 分别锁 runtime tuple、Unix recorder、artifact verification 与 pure Docker state；`9a83f3c fix(search): align local rerank runtime state` 对齐 containerd image-store、image labels、network/log 与 Moby legacy NVIDIA PID1 env，`f2ba893 feat(search): verify mounted rerank template` 增加 exact single-file tar 门，`42f6332 feat(search): add bounded Docker engine adapter` 只连本机 Unix socket并实现有界 create/inspect/event/cleanup，`99faad7 feat(search): add recording-only rerank session` 再以 replay+archive acknowledgement 建立订阅 barrier、锁 host/container artifact view，并在每次 `Record` 前后与 `Evidence` 时重新认证 same PID/static/env/UDS。卡后 recovery 系列 `3c398db`、`d0578e5`、`498b09c`、`fb4635d` 增加 root-only persistent journal、crash-temp sweep、deterministic name/spec identity 与 fsync/容量边界；`f329b32` 的初版 coordinator 在独立审查中被发现错误地把 same-name Create 当跨重启 completion fence，`cad160b` 撤掉 re-Create 后仍依赖特定 Moby inspect/register lock 将 name-visible creating 晋级 owned，二者均不作为可部署 waypoint；最终 `fc62379` 才撤掉所有 `creating` name-resolution/cleanup authority，`ea6d5e1 feat(search): wire rerank recovery journal` 随后将 durable prepared/creating/owned 顺序、启动前串行 drain、session lifetime lease 与 container→host→record→engine→journal 关闭顺序接入真实 local wrapper。终态只自动清 prepared 与 owned(ID)，任何 ambiguous creating 永久 quarantine/block 且零 Docker/host mutation。session 不实现 `Scorer`、不暴露 endpoint/key/Engine 或可反序列化 admission token，production 两道 certified gate 仍无条件 unavailable。全仓 test、package race、vet/build、linux/amd64 package build、gofmt/diff-check 全绿，独立接缝复审 PASS；但无真实 NVIDIA child/mountinfo/UDS evidence，也没有可跨 Docker 实现认证 ambiguous Create handler 完成的 barrier，故只记 provisional waypoint，不得称 terminal、automatic cleanup、no-orphan 或 production certified。
