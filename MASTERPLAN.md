...
> **状态快照**：原交接 waypoint `c7189c2`，当前开发 waypoint `99faad7`（分支 `codex/search-api-v1`）；P1 N_eff v1 五卡已于 2026-08-11 完成至 `94f9f88`，整块审查补强至 `d213376`；P2 EAV 两张校准卡已完成至 `c60ec5f`，整块审查补强至 `22c379e`，Compose 配置闭合至 `1a2c601`；P3 已完成 R-1 设计定案至 `8c83af3`、R-2 pure rerank core `c8e72f8`、R-3 strict runtime/config `9f837ee` + `c385830`（admission 边界澄清 `9697a3f`、网络审查补强 `e649afc`）、R-4 内部 relevance orchestration `dfc89c7` 与 R-5 relevance 公开契约 `ea7a87e` + `9e83c40` + `dae246d`；R-6 已落 provisional recording/replay/scorecard scaffold至 `5597b52`，R-6a 已落 local Docker Engine / recording-only session scaffold 至 `99faad7`，但**尚未完成真实 judged corpus、authenticated linux/amd64 NVIDIA recordings、live mount/UDS evidence 或 scorecard，因此 R-6/R-6a 均未通过**。Phase 0–8 与 P1/P2 代码均收口；当前晋级的 curated regression replay 成绩为 P=1.000 / R=0.996 / FP=0 / U=0.003 / silent=0、hard R=0.975（完整录制口径与残余见 §15）。R-6a scaffold 冻结时全仓 `go test ./...`、`go test -race ./...`、`go vet ./...`、`go build ./...`、linux/amd64 package build 与 `git diff --check` 全绿。
...
> **EN —** Handoff from waypoint `c7189c2`: P1 N_eff v1 and P2 EAV calibration are complete. N_eff now adds bounded content-core fingerprints, quote lineage, and additive fold-reason export on top of the existing same-root/whole-page-simhash folds. EAV now enforces verbatim extraction quotes through the golden replay gate and can opt only its operator-managed client into private networking while request BYOK remains public-only. P3's R-1 design, R-2 pure core, R-3 isolated strict runtime/config, R-4 internal relevance orchestration, and R-5 explicit relevance contract are complete. R-6 has a pinned manifest, strict offline replay/metrics, a provisional-only scorecard, and a label-blind recorder; R-6a now has a fail-closed local Docker Engine and recording-only session scaffold. Neither gate is complete without independently judged data, authenticated linux/amd64 NVIDIA recordings and live runtime evidence, price evidence, and a live 10+100 run. Production activation remains closed until both gates pass.
...
R-6a ⟳ c4580e9..99faad7：local Docker Engine controller/recording-only live session scaffold；真实 NVIDIA evidence、cleanup recovery 与 R-6 manifest 同过才生成 certified registry entry
...
> **R-6a provisional scaffold（2026-08-12，未通过 R-6a）**：`c4580e9` 先锁 local-only supervisor 与 R-6/R-6a 边界，`fda8a3f`、`a6721ea`、`155c24f`、`cd7903b` 分别锁 runtime tuple、Unix recorder、artifact verification 与 pure Docker state；本轮 `9a83f3c fix(search): align local rerank runtime state` 对齐 containerd image-store、image labels、network/log 与 Moby legacy NVIDIA PID1 env，`f2ba893 feat(search): verify mounted rerank template` 增加 exact single-file tar 门，`42f6332 feat(search): add bounded Docker engine adapter` 只连本机 Unix socket并实现有界 create/inspect/event/cleanup，`99faad7 feat(search): add recording-only rerank session` 再以 replay+archive acknowledgement 建立订阅 barrier、锁 host/container artifact view，并在每次 `Record` 前后与 `Evidence` 时重新认证 same PID/static/env/UDS。session 不实现 `Scorer`、不暴露 endpoint/key/Engine 或可反序列化 admission token，production 两道 certified gate 仍无条件 unavailable。全仓 test/race/vet/build、linux/amd64 package build、gofmt/diff-check 全绿，两路 Moby 源码交叉审查无剩余 P0/P1；但无真实 NVIDIA child/mountinfo/UDS evidence，且超过 15 秒 daemon outage 与 Create 已生效却在 ID 返回前断线仍需 journaled deterministic recovery，故只记 provisional waypoint，不得称 terminal、no-orphan 或 production certified。
