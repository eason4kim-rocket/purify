...
- R-3 在启动 sidecar/任何 recorder 读取 env value **之前**先提交并加载人工审过的 exact `env-policy`：每个允许 name 预声明为 public 或 secret、required/optional 与值约束；deployment spec 出现 unknown name 立即拒绝，绝不能先录下来再猜它是否是凭据，新增 name 必须先走独立 policy review commit。project-owned launcher 不把 pinned image 的 inherited Env 直接交给 vLLM，而是从空环境按该 policy 构造 child env；因此镜像自带的 build/usage `VLLM_*` 也会被 scrub，attestation 锁的是 **post-scrub vLLM process env**。reference policy 要求 `HF_HUB_OFFLINE=1` 与 `VLLM_NO_USAGE_STATS=1`；secret name 仅允许 required `VLLM_API_KEY`；除这三个 exact name 外，其他 `VLLM_*`、`HF_*`、`TRANSFORMERS_*`、`TOKENIZERS_*`（包括 `VLLM_USE_FASTOKENS`）一律不存在，平台所需 PATH/CUDA 等 public name/value 也必须先显式进 policy，R-3 fixture 若证明还需某项，须先回写本节和 policy 再认证。missing/wrong/duplicate telemetry opt-out 必须 fail closed，不能依赖 home 目录里的 `do_not_track` 偶然存在。`VLLM_API_KEY` raw value 必须与 Purify key 一样满足 1..16 KiB、UTF-8、无 control/首尾空白；deployment controller 必须证明两端引用同一 immutable secret ref，无法比较 ref 时只在内存做 constant-time value compare，empty/mismatch/越界都 fail closed，之后才可归一为 `<present>`。首个 fixture/manifest 固定 image/platform、template+model snapshot digests、canonical non-secret argv/argv digest 与 sidecar **全部** effective env names；public 值逐字保存，唯一 secret 只保存 `<present>`/presence。canonical env digest 锁为 `lowerhex(SHA-256("rerank-env-v1\0" || each(uvarint(len(name+"="+redactedValue)) || name+"="+redactedValue)))`，entry 按 env name 的 UTF-8 bytes 排序并拒 duplicate；占位参与 digest而真实 key永不参与。另单列 `api_auth=VLLM_API_KEY/api_key_present=true`；真实 secret 绝不进入 argv、recording、语料、日志或仓库。R-3 必须交付 project-owned deployment admission 工具：从只读 OCI/container spec、post-scrub child env 与 mounted snapshot/template 计算这些可验证值，和 committed manifest exact 比较；任一不符就终止 child、拒绝开放 Purify capability。Purify 进程本身只验证上一段的可观测 config/profile/served-model 子集，不冒充 deployment controller。fixture 与本节 nested wire 不合时先修 R-1 设计卡，不允许 adapter 自由猜；R-6 未晋级前生产 capability 始终 unavailable。
